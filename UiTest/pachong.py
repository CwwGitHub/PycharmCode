...
while True:
    #等待loading遮罩消失
    WebDriverWait(driver,5).until_not(lambda d:driver.find_element(By.ID,"el-loading-mask"))
    #获取页面上每本图书的单元格
    allSearchBooks = driver.find_elements(By.XPATH,"//div[@id='bookItem']/a")
    #收集每本图书的单元格上的书名与定价信息
    for book in allSearchBooks:
        csvRows.append([number,book.find_element(By.CLASS_NAME,"list-title").text,book.find_element(By.CLASS_NAME,"price").text])
        number = number+1
    #如果一翻到最后一页（即下一页按钮为禁用状态），则退出收集
